...
public class ElementImplDispatcher implements PageItemImplDispatcher {
    ...
    public Map<String, String> getInfo() {
        ...
        interfaceToImplMap.asMap()
                .forEach((key, value) -> res.put(key.getSimpleName(), Str.toPrettyString(value)));
        return res;
    }
    ...
    protected void loadDefault() {
        ...
        log.debug("[ElementImplDispatcher] implementation load - success : {}", Str.toPrettyString(getInfo()));
    }
    ...
    private <T extends Element, R extends BaseElement> ElementImpl<R> findByElementConfig(Class<T> elementInterface,
        ...
        if (!interfaceToImplMap.containsKey(elementInterface)) {
            throw new ElementDiscoveryException(
                    format("Interface '{}' do not exists in ElementImplDispatcher. You have to add this using #addImplementation",
        ...
        if (implementations.isEmpty()) {
            throw new ElementDiscoveryException(format(
                    "Interface '{}' do not have any implementations  in ElementImplDispatcher. You have to add using #addImplementation",
                    elementInterface.getSimpleName()
            ), this);
        }
